...
export async function uploadVehicleImage(
  ...
  const uuid = uuidv4().replace(/-/g, '');
  const fileExtension = fileName.split('.').pop() || '';
  const objectName = `vehicles/${vehicleId}/images/${uuid}.${fileExtension}`;
  ...
  try {
    ...
    // Rollback upload on error
    await rollbackUpload(objectName);
    console.error('Error uploading vehicle image to S3:', error);
    throw new Error('Failed to upload vehicle image to S3');
  }
}
...
export async function uploadShippingDocument(
  ...
  const uuid = uuidv4().replace(/-/g, '');
  const fileExtension = fileName.split('.').pop() || '';
  const objectName = `vehicles/${vehicleId}/shipping/${uuid}.${fileExtension}`;
  ...
  try {
    ...
    // Rollback upload on error
    await rollbackUpload(objectName);
    console.error('Error uploading shipping document to S3:', error);
    throw new Error('Failed to upload shipping document to S3');
  }
}
